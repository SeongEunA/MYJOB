	...
	<div class="col-12 bodyContainer">
		<div class="col-6 regCourseDiv">
			<div class="regCourseImg">
				<a href="/review/regReview"><img src="/resources/images/banner_cos_top3.gif" class="regCourseImg"></a>
			</div>
		</div>
				...
				<div>
					<table class="tableDiv">
						...
						<c:forEach items="${reviewList }" var="reviewVO">
							<tr>
								<td rowspan="2"><a
									href="/review/detailReview?reviewBoardCode=${reviewVO.reviewBoardCode }&memberId=${sessionScope.loginInfo.memberId}">
										<img
										src="/resources/images/${reviewVO.reviewImgList[0].reviewImgAttachedName }" width="250" height="250">
								</a></td>
								<td colspan="2"><a
									href="/review/detailReview?reviewBoardCode=${reviewVO.reviewBoardCode }&&memberId=${sessionScope.loginInfo.memberId}">
										${reviewVO.reviewBoardTitle }</a>
										</td>
							</tr>
							<tr>
								<td>작성일</td>
				...
				<div class="row">
					<div class="col text-center pagingDiv">
						<c:if test="${reviewVO.prev }">
							<a
								href="/review/selectReviewList?nowPage=${reviewVO.beginPage - 1 }">prev</a>
						</c:if>
						<c:forEach begin="${reviewVO.beginPage }"
							end="${reviewVO.endPage }" var="pageNumber">
							<a
								href="/review/selectReviewList?nowPage=${pageNumber }&searchKeyword=${reviewVO.searchKeyword}&searchValue=${reviewVO.searchValue}"
								<c:if test="${reviewVO.nowPage eq pageNumber }">class="selectedPage"</c:if>>
								${pageNumber }</a>
						</c:forEach>
						<c:if test="${reviewVO.next }">
							<a
								href="/review/selectReviewList?nowPage=${reviewVO.endPage + 1 }">next</a>
						</c:if>
					</div>
				</div>
				<div class="col-6 searchContainer">
					<form class="row align-items-center"
						action="/review/selectReviewList" method="post">
						<div class="col-4">
						...
						<div class="col-5">
							...
							<input type="text" name="searchValue" class="form-control"
								id="autoSizingInput" value="${reviewVO.searchValue }">
						</div>
						<div class="col-3">
